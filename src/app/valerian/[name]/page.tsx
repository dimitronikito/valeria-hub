...
const StatBar: React.FC<StatBarProps> = ({ label, value, max, color }) => (
  <div className="flex items-center space-x-2 mb-2">
    ...
    <div className="flex-grow bg-indigo-800 rounded-full h-2 sm:h-3">
      <div className={`h-2 sm:h-3 rounded-full ${color}`} style={{ width: `${(value / max) * 100}%` }}></div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-indigo-950 text-white p-4 sm:p-6 md:p-8">
      <div className="max-w-4xl mx-auto">
        <button
          onClick={handleBack}
          className="mb-4 px-3 py-1 bg-indigo-700 text-yellow-400 rounded text-sm sm:text-base transition-colors hover:bg-indigo-600"
        >
          ← Back
        </button>
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="flex flex-col sm:flex-row sm:space-x-6">
              <div className="sm:w-1/3 mb-4 sm:mb-0">
                <div className="relative w-full pb-[100%] sm:pb-[133%]">
                  <Image
                    src={valerian.image}
                    alt={valerian.name}
                    fill
              ...
              <div className="sm:w-2/3">
                <div className="flex justify-between text-sm sm:text-base mb-4">
                  <div className="flex items-center space-x-2">
                    <span className={`${typeColorClass}`}>{valerian.type}</span>
                    <ElementIcon type={valerian.type} />
                  </div>
                  <span>{valerian.class}</span>
                </div>
                ...
                <div>
                  <h2 className="text-lg sm:text-xl font-semibold mb-2 text-yellow-400">Stats</h2>
        ...
        <div className="bg-indigo-900 rounded-lg p-4 sm:p-6 shadow-lg">
